    ...
    class assembler
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                // one argument instruction checking
                ...
                //Sprawdzanie czy to nie jmp lable
                ...
                if (ri.instruction0[0] == '[')
                {
                    ...
                    if (Char.IsDigit(RI[0]))
                    {
                        if (RI.Length > 1)
                        {
                            ...
                            {
                                RI = RI[2..];
                                numeric0 = GetValueFromAddress(int.Parse(RI, System.Globalization.NumberStyles.HexNumber));

                            }
                            else
                            {
                                numeric0 = GetValueFromAddress(int.Parse(RI));
                    ...
                    else if (RI.Contains('+') || RI.Contains('-'))
                    {
                        ...
                        numeric0 = assembler.memory[iadr][index];
                    ...
                    else
                    {
                        ...
                        numeric1 = GetValueFromAddress(regvalue);
            ...
            if (!ri.instruction1.Equals(NOT_A_INSTRUCTION))
            {
                ...
                if (ri.instruction1[0] == '[')
                {
                    ...
                    if (Char.IsDigit(RI[1]))
                    {
                        ...
                        {
                            RI = RI[2..];
                            numeric1 = GetValueFromAddress(int.Parse(RI, System.Globalization.NumberStyles.HexNumber));

                        }
                        else
                        {
                            numeric1 = GetValueFromAddress(int.Parse(RI));
        ...
        public static unsafe int GetValueFromAddress(int addr)
        {
            try
            {
                int* ptr = (int*)addr;
            ...
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.Error.WriteLine("ERROR 326: Invalid Address Access => '0x{0:X}'",addr);
                Console.ResetColor();
                return 0;
        ...
        private static void Data(string arg)
        {
            ...
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.Error.WriteLine($"ERROR 325: Invalid instruction => '{name[0]}'");
                Console.ResetColor();
            }
        ...
        private static int CalcAddr(string arg)
        {
            string sadr = arg[1..^1];
            char[] ebp = new char[3] { 'e', 'b', 'p' };
